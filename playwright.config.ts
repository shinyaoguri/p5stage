...
/**
 * 種の写し (R2)。gist_id は本物と同じ 16 進 32 桁で、`e2e/seed.sql` と揃える (#81)。
 * **どの ID がどの種かは seed.sql 冒頭の対応表で引く** — 末尾 1 桁だけが違う。
 */
const SEED_OBJECTS = [
  // 公開
  ...
  // 限定公開
  ["e2e10000000000000000000000000002", SEED_REVISION, SEED_CONTENT],
  ["e2e10000000000000000000000000002", SEED_REVISION_PREV, SEED_CONTENT_PREV],
  // アセットを使う
  ["e2e10000000000000000000000000003", SEED_REVISION, SEED_ASSET_CONTENT],
  [
    "e2e10000000000000000000000000003",
    SEED_REVISION_PREV,
    SEED_ASSET_CONTENT_PREV,
  ],
  // 台帳を持たない作品 (Phase 4-1)。写しだけを置き、履歴は Cron に埋めさせる。
  ["e2e10000000000000000000000000004", SEED_REVISION, SEED_CONTENT],
  ["e2e10000000000000000000000000004", SEED_REVISION_PREV, SEED_CONTENT_PREV],
  // フォークの相手 (Phase 4-3)。偽 GitHub 側にも同じ中身を置いてあるので、
  // 作品ページで見たものがそのまま複製される。
  ["e2e10000000000000000000000000006", SEED_REVISION, SEED_CONTENT],
  ["e2e10000000000000000000000000007", SEED_REVISION, SEED_CONTENT],
  ["e2e10000000000000000000000000008", SEED_REVISION, SEED_ASSET_CONTENT],
  // 作者が GitHub 側で壊した作品 (#70)。**配信中の版はアセットを使わない方**で、
  // 偽 GitHub 側にだけ壊れたマニフェストの版がある。
  ["e2e10000000000000000000000000009", SEED_REVISION, SEED_CONTENT],
]
  .map(
