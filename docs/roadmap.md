...
- p5.js の補完 (`@types/p5` をビルド時に束ねて言語サービスへ配る。赤線を戻すのは
  設定パネルのトグルとして後から — #104) → **ADR 0021**
...
発見の線を 1 本ずつ縦に切って足す。「今」(新着)・「誰」(作者)・「何について」(タグ)・
「言葉」(検索) の 4 本で、どれもログイン無しで辿れる。

- 全体一覧 (新着) / ユーザーページ (`/@<login>`) / タグ (`/tags/<tag>`)
- **検索** (`/search`): D1 の FTS5 (trigram) に題・説明・タグの索引を置き、
  日本語でも部分一致で引く。索引の同期はトリガー → **ADR 0020**
- 残り: 一覧のページング / サイト共通ヘッダー (発見の動線として一緒に要る)
...
いいね・コメント・フォロー / チュートリアル / クラス機能 / 有料プラン /
OpenProcessing 書き出し / 外部 embed

有料プランは **ADR 0023** に原則だけ定めてある (何に課金し、何に課金しないか)。当面の
収益経路は GitHub Sponsors のみで、決済基盤の選択は保留。Phase 3 のアセットクォータが
そのまま課金の土台になるので、上限はユーザーごとの設定値として持つ。
