...
- Web Audio の user activation はクロスオリジン子フレームへ伝播しない。
  `allow="autoplay"` の委譲で足りるかは実機検証が要る、として保留していたが、
  **足りることを実測で確認した** (#32)。本体側のクリックだけでスケッチの `AudioContext` が
  `running` で始まるため、「クリックで開始」の導線は置かない。`getUserMedia` も委譲で通る。
  効き目を握るのは 1 段目 (本体 → ランナー) の委譲だけで、2 段目は `srcdoc` が親の許可を
  継承する。検証は `e2e/permissions.spec.ts`
...
## 補足 (2026-08-15 / Phase 4-4a)

**プロトコルの版は「追加で上がる」ものとして扱い、本体は範囲で受ける。**

`ready` の `protocolVersion` は当初、本体の期待と**厳密一致**で見ていた。噛み合わない
組み合わせを黙って動かさないための判定だが、この形は**デプロイの順序と両立しない**。

CI はランナー (preview) を本体 (web) より先に出す。本体を先に出すと iframe の参照先が
欠けるからで、この順は変えられない。厳密一致のままランナーの版を上げると、ランナーが
出てから本体が追いつくまでの数分、`ready` がすべて弾かれて**全スケッチが動かなくなる**。
作品ページの HTML は `s-maxage=60` で配るので、エッジに残った古い本体を掴む分だけ窓は
さらに延びる。版を上げるたびに、機能とは無関係な停止が必ず起きる形になっていた。

- 本体が見るのは `MIN_RUNNER_PROTOCOL_VERSION` **以上かどうか**だけにする
  (`isSupportedRunnerVersion`)。断るのは古すぎるランナーで、新しいランナーは受ける
- これが成り立つのは、**知らないメッセージを両側が既に捨てている**ため
  (`parseHostMessage` / `parseRunnerMessage` は未知の型に null を返す)。型を足すだけの
  変更なら、古い側は「その機能だけ効かない」で済む
- したがって `PROTOCOL_VERSION` を上げる意味も変わる。上げるのは**足したこと**の記録で、
  最低版を上げるのは**既存のメッセージの意味を変えたとき**に限る。後者は本質的に
  非互換なので、そのときだけ「先に本体を出す」段取りが要る

Phase 4-4 (サムネイル) が版 3 で `capture` / `thumbnail` を足すが、この規則の下では
ランナーが先に出ても本体が先に出ても実行は止まらない。サムネイルが撮れない時間が
数分あるだけで、それは画面上何も起きないのと同じに見える。
